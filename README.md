...
You also need to prepare your own dataset and arrange the images under [datasets](datasets) folder as below: 
...
When specified, [train.py](train.py) will initialize a light-weight [generator](generator.py) for training a CartoonGAN. This light-weight generator is designed by ourselves to minimalize inference time while achieving similar effect. We will also make adjustment to [discriminator](discriminator.py).
...
- [taki0112/CartoonGAN-Tensorflow] where we modify [edge_smooth.py](https://github.com/taki0112/CartoonGAN-Tensorflow/blob/master/edge_smooth.py) to fit our needs 
