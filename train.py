...
class Trainer:
    ...
    def _save_generated_images(self, batch_x, image_name, nrow=2, ncol=4):
        ...
        for i in range(real_nrow):
            ...
            print('cur_row', cur_row.shape)
            out_arrs.append(cur_row)
        out_arrs = tf.concat(out_arrs, 0)
        print('out_arr', out_arrs.shape)
        if not os.path.isdir(self.result_dir):
            os.makedirs(self.result_dir)
        print(out_arrs.numpy().shape)
        ...
        for epoch in range(epochs):
            ...
            with summary_writer.as_default():
                if not self.disable_sampling:
                    ...
                    img = tf.expand_dims(self._save_generated_images(
                            val_fake_batch,
                            image_name=("val_pretrain_generated_images_at_epoch_"
                                        f"{epoch_idx}_step_{step}.png")),
                            0,
                    )
                    tf.summary.image('val_generated_images', img, step=global_step)
        ...
        for epoch in range(epochs):
            ...
            with summary_writer.as_default():
                if not self.disable_sampling:
                    ...
                    img = tf.expand_dims(self._save_generated_images(
                            val_fake_batch,
                            image_name=("val_generated_images_at_epoch_"
                                        f"{epoch_idx}_step_{step}.png")),
                            0,
                    )
                    tf.summary.image('val_generated_images', img, step=global_step)
